...
# 页面基础配置
# -----------------------------------------------------------------------------
st.set_page_config(page_title="高校财政支出对比看板（2024 vs 2025）", layout="wide")
st.title("📊 高校财政支出数据看板（年度对比：2024 vs 2025）")
...
with tab1:
    ...
    with left:
        st.subheader("① 月度总支出趋势（基于所选金额类别的支出趋势）")
    ...
    with right:
        ...
        top_n = st.slider("Top N（分类）", 3, min(30, len(cat_total)), min(10, len(cat_total)))
    ...
    st.subheader("③ 分类占比（环形图展示各分类支出占比）")
    fig_pie = px.pie(cat_top, names="分类", values=amount_col, hole=0.45)
    st.plotly_chart(fig_pie, width='stretch')

    st.subheader("④ 月×分类 构成（堆叠柱状图展示月度各分类构成变化）")
...
with tab2:
    ...
    with r:
        st.subheader("② 差异贡献瀑布图（分类贡献 TopN）")
    ...
    cL, cR = st.columns(2)
    with cL:
        st.subheader("③ 增加最多 Top10（需要“用心投入”）")
    ...
    with cR:
        st.subheader("④ 减少最多 Top10（体现“开源节流”）")
...
# -----------------------------------------------------------------------------
with tab3:
    st.subheader("2025：高支出明细（TopN）+ 归因标签 + 节约证据（减少项）")
    ...
    # 2) 对应 2024 “同类参考”：按（分类 + 明细）聚合做粗对照（便于讲清楚“不是乱花钱”）
    ref_2024 = (
    ...
    save_sum = float(save_items[save_items["差额"] < 0]["差额"].sum())  # 负数合计（节约）
    inc_sum = float(by_cat[by_cat["差额"] > 0]["差额"].sum())

    m1, m2, m3, m4 = st.columns(4)
    m1.metric("2025 总额（当前筛选）", f"{df_2025[amount_col].sum():,.2f}")
    m2.metric("节约项合计（分类减少Top10）", f"{save_sum:,.2f}")
    ...
    with left:
        st.subheader("① 2025 高支出明细表（用于展示“用心投入/重点保障”）")
        show = top_detail.copy()
        show["月份"] = show["月份"].dt.strftime("%Y-%m")

        show_cols = ["月份", "分类", "明细", amount_col, "已支付", "未支付", "归因标签", "2024同类合计", "同类差额(本笔-2024同类)"]
    ...
    with right:
        st.subheader("② 高支出归因结构（证明“钱花在刀刃上”）")
        ...
        st.subheader("③ 节约项（分类减少 Top10，直接支撑开源节流）")
    ...
    st.markdown(
        f"""
    - **总体结论（口径：{amount_col}）**：2025 年相比 2024 年的总差额为 **{diff_total:,.2f}**，表明在支出结构上进行了显著优化，同时实现了合理的开源节流。
    ...
    - **节约体现（减少项，直接反映“开源节流”）**：2025 年在以下支出项上实现了节约：{_fmt_rows(top_dec)}；分类减少 Top10 合计为 **{save_sum:,.2f}**。  
